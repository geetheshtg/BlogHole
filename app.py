...
def stories():
    ...
    if result > 0:
        return render_template('articles.html', articles=stories)
    else:
...
def dashboard():
    ...
    if result > 0:
        return render_template('dashboard.html', articles=stories)
    else:
        msg = 'No Stories Found'
...
def add_story():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute("INSERT INTO articles(title, body, author, create_date) VALUES(%s, %s, %s, CURDATE())",(title, body, session['username']))
